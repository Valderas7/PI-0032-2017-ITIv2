...
df_all_merge = df_all_merge[(df_all_merge["path_n_stage"]!= 'N0') & (df_all_merge["path_n_stage"]!= 'NX') &
                            ...
                            (df_all_merge["path_n_stage"]!= 'N0 (MOL+)') & (df_all_merge["subtype"]!= 'BRCA_Normal')]

""" Se eliminan todas las columnas de mutaciones excepto la de tipo histológico """
df_all_merge = df_all_merge[['ID', 'subtype']]
...
df_all_merge = pd.get_dummies(df_all_merge, columns = ["subtype"])
...
"""
# Entrenamiento
train_lumA_tiles = train_data['subtype_BRCA_LumA'].value_counts()[1] # Pacientes con Luminal A
train_lumB_tiles = train_data['subtype_BRCA_LumB'].value_counts()[1] # Pacientes con Luminal B
train_basal_tiles = train_data['subtype_BRCA_Basal'].value_counts()[1] # Pacientes con Basal
train_her2_tiles = train_data['subtype_BRCA_Her2'].value_counts()[1] # Pacientes con Her2
print(train_lumA_tiles, train_lumB_tiles, train_basal_tiles, train_her2_tiles)

if train_her2_tiles <= (train_basal_tiles or train_lumB_tiles or train_lumA_tiles):
    difference_train = train_idc_tiles - train_ilc_tiles
    train_data = train_data.sort_values(by = 'subtype_BRCA_LumA', ascending = True)
else:
...
train_labels_subtype = train_data.iloc[:, 1:]
valid_labels_subtype = valid_data.iloc[:, 1:]
test_labels_subtype = test_data.iloc[:, 1:]

""" Para poder entrenar la red hace falta transformar los dataframes en arrays de numpy. """
train_labels_subtype = np.asarray(train_labels_subtype).astype('float32')
valid_labels_subtype = np.asarray(valid_labels_subtype).astype('float32')
test_labels_subtype = np.asarray(test_labels_subtype).astype('float32')
...
""" Instanciar lotes """
train_gen = train_aug.flow(x = train_image_data, y = train_labels_subtype, batch_size = 32)
val_gen = val_aug.flow(x = valid_image_data, y = valid_labels_subtype, batch_size = 32, shuffle = False)
...
all_model = layers.Dropout(0.5)(all_model)
subtype = layers.Dense(train_labels_subtype.shape[1], activation ="softmax", name ='subtype')(all_model)

model = keras.models.Model(inputs = base_model.input, outputs = subtype)
...
""" Esto se hace para que al hacer el entrenamiento, los pesos de las distintas salidas se balanceen """
subtype_index = np.argmax(train_labels_subtype, axis = 1)
class_weights = compute_class_weight(class_weight = 'balanced', classes = np.unique(subtype_index), y = subtype_index)
...
checkpoint_path = '/home/avalderas/img_slides/anatomical pathology/molecular subtype/inference/models/model_image_subtype_{epoch:02d}_{val_loss:.2f}.h5'
