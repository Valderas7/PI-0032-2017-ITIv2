...
df_list = [df_age, df_neoadjuvant, df_dfs_status, df_tumor_type, df_stage, df_path_t_stage, df_path_n_stage,
           df_path_m_stage, df_subtype, df_snv, df_cnv]
...
train_data, test_data = train_test_split(df_all_merge, test_size = 0.20, stratify = df_all_merge['Relapse'])
train_data, valid_data = train_test_split(train_data, test_size = 0.15, stratify = train_data['Relapse'])

""" Se iguala el número de teselas con y sin metástasis a distancia """
# Entrenamiento
train_relapse_tiles = train_data['Relapse'].value_counts()[1] # Recaídas
train_no_relapse_tiles = train_data['Relapse'].value_counts()[0] # Sin recaídas

if train_no_relapse_tiles >= train_relapse_tiles:
    difference_train = train_no_relapse_tiles - train_relapse_tiles
    train_data = train_data.sort_values(by = 'Relapse', ascending = False)
else:
    difference_train = train_relapse_tiles - train_no_relapse_tiles
    train_data = train_data.sort_values(by = 'Relapse', ascending = True)

train_data = train_data[:-difference_train]

# Validación
valid_relapse_tiles = valid_data['Relapse'].value_counts()[1] # Recaídas
valid_no_relapse_tiles = valid_data['Relapse'].value_counts()[0] # Sin recaídas

if valid_no_relapse_tiles >= valid_relapse_tiles:
    difference_valid = valid_no_relapse_tiles - valid_relapse_tiles
    valid_data = valid_data.sort_values(by = 'Relapse', ascending = False)
else:
    difference_valid = valid_relapse_tiles - valid_no_relapse_tiles
    valid_data = valid_data.sort_values(by = 'Relapse', ascending = True)

valid_data = valid_data[:-difference_valid]

# Test
test_relapse_tiles = test_data['Relapse'].value_counts()[1] # Recaídas
test_no_relapse_tiles = test_data['Relapse'].value_counts()[0] # Sin recaídas

if test_no_relapse_tiles >= test_relapse_tiles:
    difference_test = test_no_relapse_tiles - test_relapse_tiles
    test_data = test_data.sort_values(by = 'Relapse', ascending = False)
else:
    difference_test = test_relapse_tiles - test_no_relapse_tiles
    test_data = test_data.sort_values(by = 'Relapse', ascending = True)
...
train_labels_relapse = train_data.pop('Relapse')
valid_labels_relapse = valid_data.pop('Relapse')
test_labels_relapse = test_data.pop('Relapse')
...
forest.fit(train_data, train_labels_relapse)

result = permutation_importance(forest, train_data, train_labels_relapse, n_repeats = 30, random_state = 42,
                                n_jobs = 2)
...
checkpoint_path = '/home/avalderas/img_slides/clinical/data/relapse/inference/models/model_data_relapse_{epoch:02d}_{val_loss:.2f}.h5'
mcp_save = ModelCheckpoint(filepath= checkpoint_path, save_best_only = True, monitor= 'val_accuracy', mode= 'max')

""" Una vez definido y compilado el modelo, es hora de entrenarlo. """
neural_network = model.fit(x = train_data, y = train_labels_relapse, epochs = 500, verbose = 1, batch_size = 32,
                           #callbacks = mcp_save,
                           validation_data = (valid_data, valid_labels_relapse))
