...
'numpy' """
model = load_model('/home/avalderas/img_slides/clinical/data/relapse/inference/models/model_data_relapse_171_0.60.h5')

test_data = np.load('/home/avalderas/img_slides/clinical/data/relapse/inference/test data/test_data.npy')
test_labels = np.load('/home/avalderas/img_slides/clinical/data/relapse/inference/test data/test_labels.npy')
...
y_pred_relapse = np.round(model.predict(test_data))

matrix_metastasis = confusion_matrix(y_true_relapse, y_pred_relapse, labels = [0, 1])
matrix_metastasis_classes = ['Sin recidivas', 'Con recidivas']

plot_confusion_matrix(matrix_metastasis, classes = matrix_metastasis_classes, title ='Matriz de confusión de recidivas')
plt.show()
...
y_pred_prob_relapse = model.predict(test_data).ravel()
...
precision, recall, threshold_metastasis = precision_recall_curve(test_labels, y_pred_prob_relapse)
