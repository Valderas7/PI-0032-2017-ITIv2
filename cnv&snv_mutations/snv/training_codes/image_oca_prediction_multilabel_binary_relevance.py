...
test_labels = test_data.iloc[:,2:-1]
...
todas las imágenes: """
def create_model_multilabel_binary_relevance():
    ...
    model.compile(loss='binary_crossentropy', ptimizer=keras.optimizers.Adam(learning_rate=0.0001), metrics=metrics)
    model.summary()
    return model
...
KERAS_PARAMS = dict(epochs = 20, batch_size = 32, verbose = 1)

clf = BinaryRelevance(classifier=Keras(create_model_multilabel_binary_relevance(), False, KERAS_PARAMS),
                      require_dense=[True,True])
clf.fit(train_image_data, train_labels)
result = clf.predict(test_image_data)
...
model.fit(x = trainGen, epochs = 2, verbose = 1, batch_size = 32, validation_data = valGen)
...
neural_network = model.fit(x = trainGen, epochs = 10, verbose = 1, batch_size = 32, validation_data = valGen)
