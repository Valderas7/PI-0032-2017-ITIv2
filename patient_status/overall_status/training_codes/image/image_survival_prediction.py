...
""" Se extrae la columna 'os_status' del dataframe de ambos subconjuntos, puesto que ésta es la salida del modelo que 
se va a entrenar."""
train_labels = train_data.pop('os_status')
test_labels = test_data.pop('os_status')
...
checkpoint_path = 'image_model_os_status_epoch{epoch:02d}.h5'
...
plt.figure(1)
plt.plot([0, 1], [0, 1], 'k--')
plt.plot(fpr, tpr, marker = '.', label='area = {:.3f})'.format(auc))
