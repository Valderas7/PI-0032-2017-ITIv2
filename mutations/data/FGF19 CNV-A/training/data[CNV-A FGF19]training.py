...
train_data, test_data = train_test_split(df_all_merge, test_size = 0.20, stratify = df_all_merge['CNV-A FGF19'])
train_data, valid_data = train_test_split(train_data, test_size = 0.15, stratify = train_data['CNV-A FGF19'])

""" Se iguala el número de teselas con mutación y sin mutación """
# Entrenamiento
train_mutation_tiles = train_data['CNV-A FGF19'].value_counts()[1] # Con mutación
train_no_mutation_tiles = train_data['CNV-A FGF19'].value_counts()[0] # Sin mutación

if train_no_mutation_tiles >= train_mutation_tiles:
    difference_train = train_no_mutation_tiles - train_mutation_tiles
    train_data = train_data.sort_values(by = 'CNV-A FGF19', ascending = False)
else:
    difference_train = train_mutation_tiles - train_no_mutation_tiles
    train_data = train_data.sort_values(by = 'CNV-A FGF19', ascending = True)

train_data = train_data[:-difference_train]

# Validación
valid_mutation_tiles = valid_data['CNV-A FGF19'].value_counts()[1] # Con mutación
valid_no_mutation_tiles = valid_data['CNV-A FGF19'].value_counts()[0] # Sin mutación

if valid_no_mutation_tiles >= valid_mutation_tiles:
    difference_valid = valid_no_mutation_tiles - valid_mutation_tiles
    valid_data = valid_data.sort_values(by = 'CNV-A FGF19', ascending = False)
else:
    difference_valid = valid_mutation_tiles - valid_no_mutation_tiles
    valid_data = valid_data.sort_values(by = 'CNV-A FGF19', ascending = True)

valid_data = valid_data[:-difference_valid]

# Test
test_mutation_tiles = test_data['CNV-A FGF19'].value_counts()[1] # Con mutación
test_no_mutation_tiles = test_data['CNV-A FGF19'].value_counts()[0] # Sin mutación

if test_no_mutation_tiles >= test_mutation_tiles:
    difference_test = test_no_mutation_tiles - test_mutation_tiles
    test_data = test_data.sort_values(by = 'CNV-A FGF19', ascending = False)
else:
    difference_test = test_mutation_tiles - test_no_mutation_tiles
    test_data = test_data.sort_values(by = 'CNV-A FGF19', ascending = True)
...
train_labels_FGF19 = train_data.pop('CNV-A FGF19')
valid_labels_FGF19 = valid_data.pop('CNV-A FGF19')
test_labels_FGF19 = test_data.pop('CNV-A FGF19')
...
forest.fit(train_data, train_labels_FGF19)

result = permutation_importance(forest, train_data, train_labels_FGF19, n_repeats = 30, random_state = 42,
                                n_jobs = 2)
...
ax.set_title("Importancia de variables [CNV-A FGF19]")
...
checkpoint_path = '/home/avalderas/img_slides/mutations/data/FGF19 CNV-A/inference/models/model_data_FGF19_{epoch:02d}_{val_loss:.2f}.h5'
mcp_save = ModelCheckpoint(filepath = checkpoint_path, monitor = 'val_loss', mode = 'min', save_best_only = True)

""" Una vez definido y compilado el modelo, es hora de entrenarlo. """
neural_network = model.fit(x = train_data, y = train_labels_FGF19, epochs = 1000, verbose = 1, batch_size = 32,
                           #callbacks = mcp_save,
                           validation_data = (valid_data, valid_labels_FGF19))
