...
test_labels = data_inibica.pop('CNV-A FGF3')
...
model = load_model('/home/avalderas/img_slides/mutations/data/CNV-A/FGF3/inference/models/model_data_FGF3_750_0.20.h5')
...
print("\n'Loss' de mutación CNV-A de FGF3: {:.2f}\n""Sensibilidad de mutación CNV-A de FGF3: {:.2f}%\n""Precisión de "
      "mutación CNV-A de FGF3: {:.2f}%\n""Especificidad de mutación CNV-A de FGF3: {:.2f}% \n""Exactitud de mutación "
      "CNV-A de FGF3: {:.2f}%\n""AUC-ROC de mutación CNV-A de FGF3: {:.2f}\n"
      "AUC-PR de mutación CNV-A de FGF3: {:.2f}".format(results[0], results[5] * 100, results[6] * 100,
                                                         (results[3]/(results[3]+results[2])) * 100, results[7] * 100,
                                                         results[8], results[9]))

if results[5] > 0 or results[6] > 0:
    print("Valor-F de mutación CNV-A de FGF3: {:.2f}".format((2 * results[5] * results[6]) / (results[5] + results[6])))
...
plt.title('AUC-ROC curve for CNV-A mutation of FGF3 gene')
plt.legend(loc = 'best')
plt.show()
...
plt.title('AUC-PR curve for CNV-A mutation of FGF3 gene')
plt.legend(loc = 'best')
plt.show()