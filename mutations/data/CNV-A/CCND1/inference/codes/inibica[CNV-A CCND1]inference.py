...
test_labels = data_inibica_v2.pop('CNV-A CCND1')
...
model = load_model('/home/avalderas/img_slides/mutations/data/CNV-A/CCND1/inference/models/model_data_CCND1_903_0.13.h5')
...
print("\n'Loss' de mutación CNV-A de CCND1: {:.2f}\n""Sensibilidad de mutación CNV-A de CCND1: {:.2f}%\n""Precisión de "
      "mutación CNV-A de CCND1: {:.2f}%\n""Especificidad de mutación CNV-A de CCND1: {:.2f}% \n""Exactitud de mutación "
      "CNV-A de CCND1: {:.2f}%\n""AUC-ROC de mutación CNV-A de CCND1: {:.2f}\n"
      "AUC-PR de mutación CNV-A de CCND1: {:.2f}".format(results[0], results[5] * 100, results[6] * 100,
                                                         (results[3]/(results[3]+results[2])) * 100, results[7] * 100,
                                                         results[8], results[9]))

if results[5] > 0 or results[6] > 0:
    print("Valor-F de mutación CNV-A de CCND1: {:.2f}".format((2 * results[5] * results[6]) / (results[5] + results[6])))
...
plot_confusion_matrix(matrix_mutation, classes = matrix_mutation_classes, title ='Matriz de confusión [CNV-A CCND1]')
plt.show()
...
plt.title('AUC-ROC curve for CNV-A mutation of CCND1 gene')
plt.legend(loc = 'best')
plt.show()
...
plt.title('AUC-PR curve for CNV-A mutation of CCND1 gene')
plt.legend(loc = 'best')
plt.show()