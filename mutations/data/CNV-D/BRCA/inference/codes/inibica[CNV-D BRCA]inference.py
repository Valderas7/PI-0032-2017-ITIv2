...
data_inibica_saved['CNV-D BRCA'] = 0
data_inibica_saved.loc[(data_inibica_saved['CNV-D BRCA1'] == 1) | (data_inibica_saved['CNV-D BRCA2'] == 1), 'CNV-D BRCA'] = 1

""" Se eliminan las columnas de CNV-D de BRCA1 y BRCA2 y se coloca la columna creada en su lugar """
data_inibica_saved = data_inibica_saved.drop(['CNV-D BRCA1', 'CNV-D BRCA2'], axis = 1)
cols = data_inibica_saved.columns.tolist()
cols = cols[:190] + cols[-1:] + cols[190:-1]
data_inibica_saved = data_inibica_saved[cols]
...
test_labels = data_inibica_saved.pop('CNV-D BRCA')
...
model = load_model('/home/avalderas/img_slides/mutations/data/CNV-D/BRCA/inference/models/model_data_BRCA_990_0.65.h5')
...
print("\n'Loss' de mutación CNV-D de BRCA: {:.2f}\n""Sensibilidad de mutación CNV-D de BRCA: {:.2f}%\n""Precisión de "
      "mutación CNV-D de BRCA: {:.2f}%\n""Especificidad de mutación CNV-D de BRCA: {:.2f}% \n""Exactitud de mutación "
      "CNV-D de BRCA: {:.2f}%\n""AUC-ROC de mutación CNV-D de BRCA: {:.2f}\n"
      "AUC-PR de mutación CNV-D de BRCA: {:.2f}".format(results[0], results[5] * 100, results[6] * 100,
                                                         (results[3]/(results[3]+results[2])) * 100, results[7] * 100,
                                                         results[8], results[9]))

if results[5] > 0 or results[6] > 0:
    print("Valor-F de mutación CNV-D de BRCA: {:.2f}".format((2 * results[5] * results[6]) / (results[5] + results[6])))
...
plot_confusion_matrix(matrix_mutation, classes = matrix_mutation_classes, title ='Matriz de confusión [CNV-D BRCA]')
plt.show()
...
plt.title('AUC-ROC curve for CNV-D mutation of BRCA gene')
plt.legend(loc = 'best')
plt.show()
...
plt.title('AUC-PR curve for CNV-D mutation of BRCA gene')
plt.legend(loc = 'best')
plt.show()