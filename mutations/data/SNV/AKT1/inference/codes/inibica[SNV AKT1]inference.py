...
test_labels = data_inibica_v2.pop('SNV AKT1')
...
model = load_model('/home/avalderas/img_slides/mutations/data/SNV/AKT1/inference/models/model_data_AKT1_1000_0.67.h5')
...
print("\n'Loss' de mutación SNV de AKT1: {:.2f}\n""Sensibilidad de mutación SNV de AKT1: {:.2f}%\n""Precisión de "
      "mutación SNV de AKT1: {:.2f}%\n""Especificidad de mutación SNV de AKT1: {:.2f}% \n""Exactitud de mutación "
      "SNV de AKT1: {:.2f}%\n""AUC-ROC de mutación SNV de AKT1: {:.2f}\n"
      "AUC-PR de mutación SNV de AKT1: {:.2f}".format(results[0], results[5] * 100, results[6] * 100,
                                                      (results[3]/(results[3]+results[2])) * 100, results[7] * 100,
                                                      results[8], results[9]))

if results[5] > 0 or results[6] > 0:
    print("Valor-F de mutación SNV de AKT1: {:.2f}".format((2 * results[5] * results[6]) / (results[5] + results[6])))
...
plt.title('AUC-ROC curve for SNV mutation of AKT1 gene')
plt.legend(loc = 'best')
plt.show()
...
plt.title('AUC-PR curve for SNV mutation of AKT1 gene')
plt.legend(loc = 'best')
plt.show()