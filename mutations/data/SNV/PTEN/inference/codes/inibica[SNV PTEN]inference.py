...
test_labels = data_inibica_v2.pop('SNV PTEN')
...
model = load_model('/home/avalderas/img_slides/mutations/data/SNV/PTEN/inference/models/model_data_PTEN_1000_0.59.h5')
...
print("\n'Loss' de mutación SNV de PTEN: {:.2f}\n""Sensibilidad de mutación SNV de PTEN: {:.2f}%\n""Precisión de "
      "mutación SNV de PTEN: {:.2f}%\n""Especificidad de mutación SNV de PTEN: {:.2f}% \n""Exactitud de mutación "
      "SNV de PTEN: {:.2f}%\n""AUC-ROC de mutación SNV de PTEN: {:.2f}\n"
      "AUC-PR de mutación SNV de PTEN: {:.2f}".format(results[0], results[5] * 100, results[6] * 100,
                                                      (results[3]/(results[3]+results[2])) * 100, results[7] * 100,
                                                      results[8], results[9]))

if results[5] > 0 or results[6] > 0:
    print("Valor-F de mutación SNV de PTEN: {:.2f}".format((2 * results[5] * results[6]) / (results[5] + results[6])))
...
plot_confusion_matrix(matrix_mutation, classes = matrix_mutation_classes, title ='Matriz de confusión [SNV PTEN]')
plt.show()
...
plt.title('AUC-ROC curve for SNV mutation of PTEN gene')
plt.legend(loc = 'best')
plt.show()
...
plt.title('AUC-PR curve for SNV mutation of PTEN gene')
plt.legend(loc = 'best')
plt.show()