...
""" Se eliminan todas las columnas de mutaciones excepto la mutación objetivo """
df_all_merge = df_all_merge[['ID', 'SNV_AKT1']]
...
train_data, test_data = train_test_split(df_all_merge, test_size = 0.20, stratify = df_all_merge['SNV_AKT1'])
train_data, valid_data = train_test_split(train_data, test_size = 0.15, stratify = train_data['SNV_AKT1'])
...
# Entrenamiento
train_mutation_tiles = train_data['SNV_AKT1'].value_counts()[1] # Con mutación
train_no_mutation_tiles = train_data['SNV_AKT1'].value_counts()[0] # Sin mutación

if train_no_mutation_tiles >= train_mutation_tiles:
    difference_train = train_no_mutation_tiles - train_mutation_tiles
    train_data = train_data.sort_values(by = 'SNV_AKT1', ascending = False)
else:
    difference_train = train_mutation_tiles - train_no_mutation_tiles
    train_data = train_data.sort_values(by = 'SNV_AKT1', ascending = True)

train_data = train_data[:-difference_train]

# Validación
valid_mutation_tiles = valid_data['SNV_AKT1'].value_counts()[1] # Con mutación
valid_no_mutation_tiles = valid_data['SNV_AKT1'].value_counts()[0] # Sin mutación

if valid_no_mutation_tiles >= valid_mutation_tiles:
    difference_valid = valid_no_mutation_tiles - valid_mutation_tiles
    valid_data = valid_data.sort_values(by = 'SNV_AKT1', ascending = False)
else:
    difference_valid = valid_mutation_tiles - valid_no_mutation_tiles
    valid_data = valid_data.sort_values(by = 'SNV_AKT1', ascending = True)

valid_data = valid_data[:-difference_valid]

# Test
test_mutations_tiles = test_data['SNV_AKT1'].value_counts()[1] # Con mutación
test_no_mutations_tiles = test_data['SNV_AKT1'].value_counts()[0] # Sin mutación

if test_no_mutations_tiles >= test_mutations_tiles:
    difference_test = test_no_mutations_tiles - test_mutations_tiles
    test_data = test_data.sort_values(by = 'SNV_AKT1', ascending = False)
else:
    difference_test = test_mutations_tiles - test_no_mutations_tiles
    test_data = test_data.sort_values(by = 'SNV_AKT1', ascending = True)
...
train_labels = train_data.pop('SNV_AKT1')
valid_labels = valid_data.pop('SNV_AKT1')
test_labels = test_data.pop('SNV_AKT1')
...
checkpoint_path = '/home/avalderas/img_slides/mutations/image/AKT1 SNV/inference/models/model_image_AKT1_{epoch:02d}_{val_loss:.2f}.h5'
mcp_loss = ModelCheckpoint(filepath = checkpoint_path, monitor = 'val_loss', mode = 'min', save_best_only = True)

""" Una vez definido el modelo, se entrena: """
model.fit(x = train_gen, epochs = 15, verbose = 1, validation_data = val_gen,
