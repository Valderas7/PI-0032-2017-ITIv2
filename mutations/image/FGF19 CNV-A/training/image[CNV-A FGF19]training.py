...
df_all_merge = df_all_merge[['ID', 'CNV_FGF19_AMP']]
...
train_data, test_data = train_test_split(df_all_merge, test_size = 0.20, stratify = df_all_merge['CNV_FGF19_AMP'])
train_data, valid_data = train_test_split(train_data, test_size = 0.15, stratify = train_data['CNV_FGF19_AMP'])
...
# Entrenamiento
train_mutation_tiles = train_data['CNV_FGF19_AMP'].value_counts()[1] # Con mutación
train_no_mutation_tiles = train_data['CNV_FGF19_AMP'].value_counts()[0] # Sin mutación

if train_no_mutation_tiles >= train_mutation_tiles:
    difference_train = train_no_mutation_tiles - train_mutation_tiles
    train_data = train_data.sort_values(by = 'CNV_FGF19_AMP', ascending = False)
else:
    difference_train = train_mutation_tiles - train_no_mutation_tiles
    train_data = train_data.sort_values(by = 'CNV_FGF19_AMP', ascending = True)

train_data = train_data[:-difference_train]

# Validación
valid_mutation_tiles = valid_data['CNV_FGF19_AMP'].value_counts()[1] # Con mutación
valid_no_mutation_tiles = valid_data['CNV_FGF19_AMP'].value_counts()[0] # Sin mutación

if valid_no_mutation_tiles >= valid_mutation_tiles:
    difference_valid = valid_no_mutation_tiles - valid_mutation_tiles
    valid_data = valid_data.sort_values(by = 'CNV_FGF19_AMP', ascending = False)
else:
    difference_valid = valid_mutation_tiles - valid_no_mutation_tiles
    valid_data = valid_data.sort_values(by = 'CNV_FGF19_AMP', ascending = True)

valid_data = valid_data[:-difference_valid]

# Test
test_mutations_tiles = test_data['CNV_FGF19_AMP'].value_counts()[1] # Con mutación
test_no_mutations_tiles = test_data['CNV_FGF19_AMP'].value_counts()[0] # Sin mutación

if test_no_mutations_tiles >= test_mutations_tiles:
    difference_test = test_no_mutations_tiles - test_mutations_tiles
    test_data = test_data.sort_values(by = 'CNV_FGF19_AMP', ascending = False)
else:
    difference_test = test_mutations_tiles - test_no_mutations_tiles
    test_data = test_data.sort_values(by = 'CNV_FGF19_AMP', ascending = True)
...
train_labels_FGF19 = train_data.pop('CNV_FGF19_AMP')
valid_labels_FGF19 = valid_data.pop('CNV_FGF19_AMP')
test_labels_FGF19 = test_data.pop('CNV_FGF19_AMP')
...
train_labels_FGF19 = np.asarray(train_labels_FGF19).astype('float32')
valid_labels_FGF19 = np.asarray(valid_labels_FGF19).astype('float32')
test_labels_FGF19 = np.asarray(test_labels_FGF19).astype('float32')
...
""" Instanciar lotes """
train_gen = train_aug.flow(x = train_image_data, y = train_labels_FGF19, batch_size = 32)
val_gen = val_aug.flow(x = valid_image_data, y = valid_labels_FGF19, batch_size = 32, shuffle = False)
...
checkpoint_path = '/home/avalderas/img_slides/mutations/image/FGF19 CNV-A/inference/models/model_image_FGF19_{epoch:02d}_{val_loss:.2f}.h5'
