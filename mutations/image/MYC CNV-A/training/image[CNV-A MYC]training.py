...
df_all_merge = df_all_merge[['ID', 'CNV_MYC_AMP']]
...
train_data, test_data = train_test_split(df_all_merge, test_size = 0.20, stratify = df_all_merge['CNV_MYC_AMP'])
train_data, valid_data = train_test_split(train_data, test_size = 0.15, stratify = train_data['CNV_MYC_AMP'])
...
# Entrenamiento
train_myc_tiles = train_data['CNV_MYC_AMP'].value_counts()[1] # Con mutación
train_no_myc_tiles = train_data['CNV_MYC_AMP'].value_counts()[0] # Sin mutación

if train_no_myc_tiles >= train_myc_tiles:
    difference_train = train_no_myc_tiles - train_myc_tiles
    train_data = train_data.sort_values(by = 'CNV_MYC_AMP', ascending = False)
else:
    difference_train = train_myc_tiles - train_no_myc_tiles
    train_data = train_data.sort_values(by = 'CNV_MYC_AMP', ascending = True)

train_data = train_data[:-difference_train]

# Validación
valid_myc_tiles = valid_data['CNV_MYC_AMP'].value_counts()[1] # Con mutación
valid_no_myc_tiles = valid_data['CNV_MYC_AMP'].value_counts()[0] # Sin mutación

if valid_no_myc_tiles >= valid_myc_tiles:
    difference_valid = valid_no_myc_tiles - valid_myc_tiles
    valid_data = valid_data.sort_values(by = 'CNV_MYC_AMP', ascending = False)
else:
    difference_valid = valid_myc_tiles - valid_no_myc_tiles
    valid_data = valid_data.sort_values(by = 'CNV_MYC_AMP', ascending = True)

valid_data = valid_data[:-difference_valid]

# Test
test_myc_tiles = test_data['CNV_MYC_AMP'].value_counts()[1] # Con mutación
test_no_myc_tiles = test_data['CNV_MYC_AMP'].value_counts()[0] # Sin mutación

if test_no_myc_tiles >= test_myc_tiles:
    difference_test = test_no_myc_tiles - test_myc_tiles
    test_data = test_data.sort_values(by = 'CNV_MYC_AMP', ascending = False)
else:
    difference_test = test_myc_tiles - test_no_myc_tiles
    test_data = test_data.sort_values(by = 'CNV_MYC_AMP', ascending = True)
...
""" Se extraen las etiquetas de salida para cada la mutación CNV-A de ERBB2 """
train_labels_myc = train_data.iloc[:, -1]
valid_labels_myc = valid_data.iloc[:, -1]
test_labels_myc = test_data.iloc[:, -1]
...
checkpoint_path = '/home/avalderas/img_slides/mutations/image/MYC CNV-A/inference/models/model_image_myc_{epoch:02d}_{val_loss:.2f}.h5'
...
model.fit(x = train_image_data, y = train_labels_myc, epochs = 2, verbose = 1,
          validation_data = (valid_image_data, valid_labels_myc),
...
neural_network = model.fit(x = train_image_data, y = train_labels_myc,
                           epochs = 30, verbose = 1, validation_data = (valid_image_data, valid_labels_myc),
