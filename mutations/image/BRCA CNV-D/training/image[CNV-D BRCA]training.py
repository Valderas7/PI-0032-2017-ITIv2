...
""" Se eliminan todas las columnas de mutaciones excepto las que interesa"""
df_all_merge = df_all_merge[['ID', 'CNV_BRCA1_DEL', 'CNV_BRCA2_DEL']]

""" Se crea una nueva columna para recopilar las mutaciones de CNV-D de BRCA1 y BRCA2 """
df_all_merge['CNV-D BRCA'] = 0
df_all_merge.loc[(df_all_merge['CNV_BRCA1_DEL'] == 1) | (df_all_merge['CNV_BRCA2_DEL'] == 1), 'CNV-D BRCA'] = 1

""" Se eliminan las columnas de CNV-D de BRCA1 y BRCA2 """
df_all_merge = df_all_merge.drop(['CNV_BRCA1_DEL', 'CNV_BRCA2_DEL'], axis = 1)
...
train_data, test_data = train_test_split(df_all_merge, test_size = 0.20, stratify = df_all_merge['CNV-D BRCA'])
train_data, valid_data = train_test_split(train_data, test_size = 0.15, stratify = train_data['CNV-D BRCA'])
...
# Entrenamiento
train_mutation_tiles = train_data['CNV-D BRCA'].value_counts()[1] # Con mutación
train_no_mutation_tiles = train_data['CNV-D BRCA'].value_counts()[0] # Sin mutación

if train_no_mutation_tiles >= train_mutation_tiles:
    difference_train = train_no_mutation_tiles - train_mutation_tiles
    train_data = train_data.sort_values(by = 'CNV-D BRCA', ascending = False)
else:
    difference_train = train_mutation_tiles - train_no_mutation_tiles
    train_data = train_data.sort_values(by = 'CNV-D BRCA', ascending = True)

train_data = train_data[:-difference_train]

# Validación
valid_mutation_tiles = valid_data['CNV-D BRCA'].value_counts()[1] # Con mutación
valid_no_mutation_tiles = valid_data['CNV-D BRCA'].value_counts()[0] # Sin mutación

if valid_no_mutation_tiles >= valid_mutation_tiles:
    difference_valid = valid_no_mutation_tiles - valid_mutation_tiles
    valid_data = valid_data.sort_values(by = 'CNV-D BRCA', ascending = False)
else:
    difference_valid = valid_mutation_tiles - valid_no_mutation_tiles
    valid_data = valid_data.sort_values(by = 'CNV-D BRCA', ascending = True)

valid_data = valid_data[:-difference_valid]

# Test
test_mutations_tiles = test_data['CNV-D BRCA'].value_counts()[1] # Con mutación
test_no_mutations_tiles = test_data['CNV-D BRCA'].value_counts()[0] # Sin mutación

if test_no_mutations_tiles >= test_mutations_tiles:
    difference_test = test_no_mutations_tiles - test_mutations_tiles
    test_data = test_data.sort_values(by = 'CNV-D BRCA', ascending = False)
else:
    difference_test = test_mutations_tiles - test_no_mutations_tiles
    test_data = test_data.sort_values(by = 'CNV-D BRCA', ascending = True)
...
train_labels = train_data.pop('CNV-D BRCA')
valid_labels = valid_data.pop('CNV-D BRCA')
test_labels = test_data.pop('CNV-D BRCA')
...
""" Instanciar lotes """
train_gen = train_aug.flow(x = train_image_data, y = train_labels, batch_size = 32)
val_gen = val_aug.flow(x = valid_image_data, y = valid_labels, batch_size = 32, shuffle = False)
...
all_model = layers.Dropout(0.5)(all_model)
output = layers.Dense(1, activation="sigmoid", name='output')(all_model)
...
checkpoint_path = '/home/avalderas/img_slides/mutations/image/BRCA CNV-D/inference/models/model_image_BRCA_{epoch:02d}_{val_loss:.2f}.h5'
mcp_loss = ModelCheckpoint(filepath = checkpoint_path, monitor = 'val_loss', mode = 'min', save_best_only = True)

""" Una vez definido el modelo, se entrena: """
model.fit(x = train_gen, epochs = 7, verbose = 1, validation_data = val_gen,
