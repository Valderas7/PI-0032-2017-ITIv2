...
train_data, test_data = train_test_split(df_all_merge, test_size = 0.20, stratify = df_all_merge['SNV TP53'])
train_data, valid_data = train_test_split(train_data, test_size = 0.15, stratify = train_data['SNV TP53'])
...
# Entrenamiento
train_tp53_tiles = train_data['SNV TP53'].value_counts()[1] # Con mutación
train_no_tp53_tiles = train_data['SNV TP53'].value_counts()[0] # Sin mutación

if train_no_tp53_tiles >= train_tp53_tiles:
    difference_train = train_no_tp53_tiles - train_tp53_tiles
    train_data = train_data.sort_values(by = 'SNV TP53', ascending = False)
else:
    difference_train = train_tp53_tiles - train_no_tp53_tiles
    train_data = train_data.sort_values(by = 'SNV TP53', ascending = True)

train_data = train_data[:-difference_train]

# Validación
valid_tp53_tiles = valid_data['SNV TP53'].value_counts()[1] # Con mutación
valid_no_tp53_tiles = valid_data['SNV TP53'].value_counts()[0] # Sin mutación

if valid_no_tp53_tiles >= valid_tp53_tiles:
    difference_valid = valid_no_tp53_tiles - valid_tp53_tiles
    valid_data = valid_data.sort_values(by = 'SNV TP53', ascending = False)
else:
    difference_valid = valid_tp53_tiles - valid_no_tp53_tiles
    valid_data = valid_data.sort_values(by = 'SNV TP53', ascending = True)

valid_data = valid_data[:-difference_valid]

# Test
test_tp53_tiles = test_data['SNV TP53'].value_counts()[1] # Con mutación
test_no_tp53_tiles = test_data['SNV TP53'].value_counts()[0] # Sin mutación

if test_no_tp53_tiles >= test_tp53_tiles:
    difference_test = test_no_tp53_tiles - test_tp53_tiles
    test_data = test_data.sort_values(by = 'SNV TP53', ascending = False)
else:
    difference_test = test_tp53_tiles - test_no_tp53_tiles
    test_data = test_data.sort_values(by = 'SNV TP53', ascending = True)
...
train_labels_tp53 = train_data.pop('SNV TP53')
valid_labels_tp53 = valid_data.pop('SNV TP53')
test_labels_tp53 = test_data.pop('SNV TP53')
...
np.save('test_labels', test_labels_tp53)
...
forest.fit(train_data, train_labels_tp53)

result = permutation_importance(forest, train_data, train_labels_tp53, n_repeats = 30, random_state = 42,
                                n_jobs = 2)
...
checkpoint_path = '/mutations/image&data/TP53/inference/models/model_image&data_tp53_{epoch:02d}_{val_loss:.2f}.h5'
