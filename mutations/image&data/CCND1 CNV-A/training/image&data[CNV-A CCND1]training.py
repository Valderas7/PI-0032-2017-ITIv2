...
train_data, test_data = train_test_split(df_all_merge, test_size = 0.20, stratify = df_all_merge['CNV-A CCND1'])
train_data, valid_data = train_test_split(train_data, test_size = 0.15, stratify = train_data['CNV-A CCND1'])
...
# Entrenamiento
train_CCND1_tiles = train_data['CNV-A CCND1'].value_counts()[1] # Con mutación
train_no_CCND1_tiles = train_data['CNV-A CCND1'].value_counts()[0] # Sin mutación

if train_no_CCND1_tiles >= train_CCND1_tiles:
    difference_train = train_no_CCND1_tiles - train_CCND1_tiles
    train_data = train_data.sort_values(by = 'CNV-A CCND1', ascending = False)
else:
    difference_train = train_CCND1_tiles - train_no_CCND1_tiles
    train_data = train_data.sort_values(by = 'CNV-A CCND1', ascending = True)

train_data = train_data[:-difference_train]

# Validación
valid_CCND1_tiles = valid_data['CNV-A CCND1'].value_counts()[1] # Con mutación
valid_no_CCND1_tiles = valid_data['CNV-A CCND1'].value_counts()[0] # Sin mutación

if valid_no_CCND1_tiles >= valid_CCND1_tiles:
    difference_valid = valid_no_CCND1_tiles - valid_CCND1_tiles
    valid_data = valid_data.sort_values(by = 'CNV-A CCND1', ascending = False)
else:
    difference_valid = valid_CCND1_tiles - valid_no_CCND1_tiles
    valid_data = valid_data.sort_values(by = 'CNV-A CCND1', ascending = True)

valid_data = valid_data[:-difference_valid]

# Test
test_CCND1_tiles = test_data['CNV-A CCND1'].value_counts()[1] # Con mutación
test_no_CCND1_tiles = test_data['CNV-A CCND1'].value_counts()[0] # Sin mutación

if test_no_CCND1_tiles >= test_CCND1_tiles:
    difference_test = test_no_CCND1_tiles - test_CCND1_tiles
    test_data = test_data.sort_values(by = 'CNV-A CCND1', ascending = False)
else:
    difference_test = test_CCND1_tiles - test_no_CCND1_tiles
    test_data = test_data.sort_values(by = 'CNV-A CCND1', ascending = True)
...
train_labels_CCND1 = train_data.pop('CNV-A CCND1')
valid_labels_CCND1 = valid_data.pop('CNV-A CCND1')
test_labels_CCND1 = test_data.pop('CNV-A CCND1')
...
forest.fit(train_data, train_labels_CCND1)

result = permutation_importance(forest, train_data, train_labels_CCND1, n_repeats = 30, random_state = 42,
                                n_jobs = 2)
...
ax.set_title("Importancia de variables [CNV-A CCND1]")
...
checkpoint_path = '/home/avalderas/img_slides/mutations/image&data/CCND1 CNV-A/inference/models/model_image&data_CCND1_{epoch:02d}_{val_loss:.2f}.h5'
...
model.fit(x = [train_data, train_image_data], y = train_labels_CCND1, epochs = 2, verbose = 1,
          validation_data = ([valid_data, valid_image_data], valid_labels_CCND1),
...
neural_network = model.fit(x = [train_data, train_image_data], y = train_labels_CCND1, epochs = 20, verbose = 1,
                           validation_data = ([valid_data, valid_image_data], valid_labels_CCND1),
