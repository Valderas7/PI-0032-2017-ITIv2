...
""" Se eliminan las columnas de TNM (ya se tiene la columna STAGE que da la misma información) y de recidivas """
df_all_merge = df_all_merge.drop(columns = ['path_t_stage', 'path_n_stage', 'path_m_stage', 'dfs_status'])
...
cols = df_all_merge.columns.tolist()
cols = cols[:186] + cols[-1:] + cols[186:-1]
df_all_merge = df_all_merge[cols]
...
# Test
test_mutation_tiles = test_data['CNV-D BRCA'].value_counts()[1] # Con mutación
test_no_mutation_tiles = test_data['CNV-D BRCA'].value_counts()[0] # Sin mutación
...
ax.set_title("Importancia de variables [CNV-D BRCA]")
...
checkpoint_path = '/home/avalderas/img_slides/mutations/image&data/BRCA CNV-D/inference/models/model_image&data_BRCA_{epoch:02d}_{val_loss:.2f}.h5'
mcp_loss = ModelCheckpoint(filepath = checkpoint_path, monitor = 'val_loss', mode = 'min', save_best_only = True)

""" Una vez definido el modelo, se entrena: """
model.fit(x = [train_data, train_image_data], y = train_labels, epochs = 7, verbose = 1,
